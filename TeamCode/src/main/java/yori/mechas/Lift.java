...
public class Lift {
    ...
    private double liftLpos;
    private double liftRpos;
...
//        }
        ...
        liftLpos = liftLmotor.getCurrentPosition();
        liftRpos = liftRmotor.getCurrentPosition();
...
//        telemetry.addData("FF_RIGHT_POWER", ffR);
    ...
    public double getNormalizedMotorPos() {
        if (liftTarget >= Math.max(liftLpos, liftRpos)) {
            return Math.min(liftLpos, liftRpos);
        }

        return Math.max(liftLpos, liftRpos);
    }
